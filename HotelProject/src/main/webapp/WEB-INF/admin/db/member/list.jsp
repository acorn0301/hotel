		...
		<div class="dashboard_div_table">
			<ul>
				...
				<c:forEach items="${list}" var="dto">
					<li>
						<div onClick="location.href='adminMemberDetail.do?member_num=${dto.member_num}'">
	...
	<div class="text-center">
		<ul class="pagination">
			<c:if test="${startPage > 1 }">
				<li><a href="adminMemberList.do?pageNum=${1}">처음으로</a>
			</c:if>
			<c:if test="${startPage > 1 }">
				<li><a href="adminMemberList.do?pageNum=${startPage-1 }"><-</a>
			</c:if>
			<c:forEach var="pp" begin="${startPage}" end="${endPage}" varStatus="i">
				<c:if test="${pp==currentPage }">
					<li><a style="color:red" href="adminMemberList.do?pageNum=${pp }">${pp }</a></li>
				</c:if>
				<c:if test="${pp!=currentPage }">
					<li><a href="adminMemberList.do?pageNum=${pp }">${pp }</a></li>
				</c:if>
			</c:forEach>
			<c:if test="${endPage < totalPage}">
				<li><a href="adminMemberList.do?pageNum=${endPage+1}">-></a>
			</c:if>
			<c:if test="${endPage < totalPage }">
				<li><a href="adminMemberList.do?pageNum=${totalPage}">끝으로</a>
			</c:if>
		</ul>
	</div>
	<!-- 페이징 끝 -->
