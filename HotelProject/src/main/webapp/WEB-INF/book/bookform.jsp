...
<div id="hjBody">
			...
			<div class="reservationBorder">
				...
				<form action="booking.do" method="post">
					...
					<span class="smalltext">체크인</span>
						...
						<c:if test="${fbdto.hotel_num != null }">
							<input type="text" name="check_in" required="required" class="rightFloat" id="datepicker1" value="${fbdto.check_in}">
						</c:if>
						<!-- 2. 첫 접속 시 -->
						<c:if test="${fbdto.hotel_num == null }">
							<input type="text" name="check_in" required="required" class="rightFloat" id="datepicker1" placeholder="체크인 날짜 선택">
						</c:if>
					<hr>
					...
					<span class="smalltext">체크아웃</span>
						...
						<!-- 1. 다른페이지에서 넘어왔을 때 값 유지 -->
						<c:if test="${fbdto.hotel_num != null }">
							<input type="text" name="check_out" required="required" class="rightFloat" id="datepicker2" value="${fbdto.check_out}">
						</c:if>
						<!-- 2. 첫 접속 시 -->
						<c:if test="${fbdto.hotel_num == null }">
							<input type="text" name="check_out" required="required" class="rightFloat" id="datepicker2" placeholder="체크아웃 날짜 선택">
						</c:if>
						...
						<div class="roomListContents">
							...
							<form action="booking.do" method="post">
								<div class="roomInfoSpan">
									...
									<!-- 객실 넘버 -->
									<span class="roomListNum">${fbdto.room_num }</span>
					...
					<div id="aboutRoomModalborder">
						<div id="aboutRoomModal">
							...
							<div class="aboutRoomEx">
							 ...
							 1st HOTEL은 고객의 니즈를 먼저 이해하여,
							 고객에게 진정으로 의미있는 상품과 서비스를 엄선해 제공합니다.
							  </p>
							</div>
	...
	<c:if test="${step==2 }">
		<div id="step3Page">
			...
			<h1>옵션</h1>
			<div class="caption">객실 이용에 필요한 <b>옵션 사항</b>을 선택하세요.</div>
			...
			<c:if test="${fbdto.head_count >2 }">
				<div class="admin_title_div">
					<div class="admin_title_text">침대 추가&nbsp;&nbsp;<span class="smalltext">40,000원</span></div>
				</div>
			...
			<form action="insertNC.do" method="post">
				...
				<div class="reservationBorder">
					<div class="reservationContents">
						...
						<input type="text" placeholder="이름 입력" required="required" name="name" id="ncname" size="15">
						
						<br>
						
						<span class="smalltext noaccountSub">전화번호</span>
						<input type="text" placeholder="전화번호 입력" required="required" name="phone" id="ncphone" size="15">
					</div>
				</div>	
		...
		<%-- <c:if test="${member_num!=null }"> --%>
			...
			<form action="insert.do" method="post">
				...
				<div class="reservationBorder">
					<div class="reservationContents">
						...
						<div>
							...
							<c:if test="${sessionScope.fbdto.add_bed!= 0}">
								<span class="contents"><span id="add_bed_value" name="add_bed">${fbdto.add_bed}개</span></span>
						...
						<!-- 침대추가나 조식인원이 있을 시 표시 (없으면 객실가격이 곧 total가격이므로 표시할 필요없음) -->
						<c:if test="${sessionScope.fbdto.add_bed!=0 or sessionScope.fbdto.breakfast_count!=0}">
						<hr>
							<!-- 기본 객실금액 표시 -->
							<div>
								<span class="smalltext noaccountSub">객실 금액</span>
						...
						<c:if test="${sessionScope.fbdto.add_bed!=0}">
							<div>
								<span class="smalltext noaccountSub">침대 추가 금액</span>
								<span class="contents">${sessionScope.fbdto.add_bed*40000}원</span>
							</div>
						</c:if>
						...
						<c:if test="${sessionScope.fbdto.breakfast_count!=0}">
							<div>
								<span class="smalltext noaccountSub">조식 금액</span>
								<span class="contents">${sessionScope.fbdto.breakfast_count*25000}원</span>
							</div>
						</c:if>
						...
						<div class="totalPriceDiv">
							<span class="smalltext noaccountSub">요금 합계</span>
							<span id="totalPrice" class="contents">${sessionScope.fbdto.price+sessionScope.fbdto.add_bed*40000+sessionScope.fbdto.breakfast_count*25000} 원</span>
						</div>
					</div>
				</div>
				...
				<div>
					<span class="smalltext noaccountSub requestSub">요청사항</span>
					<div class="textareaDiv">
						<textarea rows="4" placeholder="${sessionScope.fbdto.book_memo }" name="book_memo" value="requestContents" readonly="readonly"></textarea>
						<!-- 요청사항(book_memo) 값 넘기기 -->
						<input type="hidden" name="book_memo" value="${sessionScope.fbdto.book_memo}">
					</div>
				</div>
